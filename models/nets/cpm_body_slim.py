...
class CPM_Model(object):
    def __init__(self, input_size, heatmap_size, stages, joints, batch_size, img_type='RGB', is_training=True):
        self.stages = stages
        self.stage_heatmap = []
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('pooled_center_map'):
            self.center_map = slim.avg_pool2d(self.cmap_placeholder,
                                              [9, 9], stride=8,
                                              padding='SAME',
                                              scope='center_map')
        """
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            with tf.variable_scope('sub_stages'):
                net = slim.conv2d(self.input_images, 128, [9, 9], scope='sub_conv1')
                net = slim.max_pool2d(net, [3, 3], padding='SAME', scope='sub_pool1')
                net = slim.conv2d(net, 128, [9, 9], scope='sub_conv2')
                net = slim.max_pool2d(net, [3, 3], padding='SAME', scope='sub_pool2')
                net = slim.conv2d(net, 128, [9, 9], scope='sub_conv3')
                net = slim.max_pool2d(net, [3, 3], padding='SAME', scope='sub_pool3')
                self.sub_stage_img_feature = slim.conv2d(net, 32, [5, 5], scope='sub_stage_img_feature')
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            with tf.variable_scope('stage_1'):
                conv1_net = slim.conv2d(self.input_images, 128, [9, 9], scope='conv1')
                conv1_net = slim.max_pool2d(conv1_net, [3, 3], padding='SAME', scope='pool1')
                conv1_net = slim.conv2d(conv1_net, 128, [9, 9], scope='conv2')
                conv1_net = slim.max_pool2d(conv1_net, [3, 3], padding='SAME', scope='pool2')
                conv1_net = slim.conv2d(conv1_net, 128, [9, 9], scope='conv3')
                conv1_net = slim.max_pool2d(conv1_net, [3, 3], padding='SAME', scope='pool3')
                conv1_net = slim.conv2d(conv1_net, 32, [5, 5], scope='conv4')
                conv1_net = slim.conv2d(conv1_net, 512, [9, 9], scope='conv5')
                conv1_net = slim.conv2d(conv1_net, 512, [1, 1], scope='conv6')
                self.stage_heatmap.append(slim.conv2d(conv1_net, self.joints, [1, 1],
                                                      scope='stage_heatmap'))
    ...
    def build_loss(self, lr, lr_decay_rate, lr_decay_step, optimizer='Adam'):
        ...
        with tf.variable_scope('train'):
            ...
            tf.summary.scalar('learning rate', self.cur_lr)
